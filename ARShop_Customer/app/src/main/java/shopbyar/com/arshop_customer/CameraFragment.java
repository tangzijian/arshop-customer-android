...
public class CameraFragment extends Fragment implements View.OnClickListener {
    ...
    private final ImageReader.OnImageAvailableListener mOnImageAvailableListener
        ...
        public void onImageAvailable(ImageReader reader) {
            ...
            mBackgroundHandler.post(new ImageSaver(image, mFolderName, mFileName,
                    getActivity()));
        }

    };

    private void setCameraControlNormal() {
        Log.i(TAG, "setCameraControlNormal:clicked");
        mUIHandler.post(new Runnable() {
            @Override
            public void run() {
                mRootView.findViewById(R.id.camera_control).setBackgroundColor(
                        Color.argb(0xcc, 0x42, 0x85, 0xf4));
                mCameraControlState = "normal";
                mUIHandler.removeCallbacks(mCorrectColor);
            }
        });
    }
    ...
    private CameraCaptureSession.CaptureCallback mCaptureCallback
        ...
        public void onCaptureCompleted(CameraCaptureSession session, CaptureRequest request,
            ...
            process(result);
            // Log.i(TAG, "onCaptureCompleted:"+(result.getRequest()));
        }
...
//        if (CameraConfigurationStore.isSetConfig()) {
//            return CameraConfigurationStore.getConfigSize();
//        }
//        else {
//            CameraConfigurationStore.setSizes(choices);
//        }
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        mRootView = inflater.inflate(R.layout.fragment_camera2_basic, container, false);
        return mRootView;
    }
    ...
    public void onResume() {
        ...
        if (mTextureView.isAvailable()) {
            Log.i(TAG, "open camera with width:" + mTextureView.getWidth() + "|height:" +
                    mTextureView.getHeight());
            openCamera(mTextureView.getWidth(), mTextureView.getHeight());
        } else {
            mTextureView.setSurfaceTextureListener(mSurfaceTextureListener);
        }
        Toast toast = Toast.makeText(getActivity(), "Snap a photo to start.", Toast.LENGTH_LONG);
...
//                if (CameraConfigurationStore.isSetConfig()) {
//                    largest = CameraConfigurationStore.getConfigSize();
//                }
                Log.i(TAG, "setUpCameraOutputs: ImageReaderSize:"+largest);
                ...
                Log.i(TAG, "setUPCameraOutputs: previewSize:"+mPreviewSize);
...
//            if (CameraConfigurationStore.isSetConfig()) {
//                texture.setDefaultBufferSize(CameraConfigurationStore.getConfigSize().getWidth(),
//                        CameraConfigurationStore.getConfigSize().getHeight());
//            }
//            else {
...
//            }
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            case R.id.control_take_picture: {
                // take button to red color
    ...
    private boolean isProcessingPicture() {
        if (mCameraControlState.equals("red")) {
            return true;
        }
        return false;
    }

    private String mCameraControlState = "normal";
    private Runnable mCorrectColor = null;
    private void setCameraControlRed() {
        Log.i(TAG, "setCameraControlRed:clicked");
        mUIHandler.post(new Runnable() {
            @Override
            public void run() {
                mRootView.findViewById(R.id.camera_control).setBackgroundColor(Color.RED);
                mCameraControlState = "red";
            }
        });

        mUIHandler.removeCallbacks(mCorrectColor);
        mCorrectColor = new Runnable() {
            @Override
            public void run() {
                mRootView.findViewById(R.id.camera_control).setBackgroundColor(
                        Color.argb(0xcc, 0x42, 0x85, 0xf4));
                mCameraControlState = "normal";
                showToast("Image not returned by camera in 3 seconds");
            }
        };
        mUIHandler.postDelayed(mCorrectColor, 3000);
    }
    ...
    public static class ImageSaver implements Runnable {
        ...
        /**
         * The folder, into which the image is saved
         */
        ...
        public ImageSaver(Image image, String folder, String name, Activity a) {
            mImage = image;
            mFolder = folder;
            mName = name;
            mActivity = a;
...
//            mCallback.onPhotoSaved(imageFile, metaFile);
        ...
        private boolean existOrCreateFolder() {
            File imageFileFolder = new File(mActivity.getExternalFilesDir(null), mFolder);
            if (!imageFileFolder.exists()) {
                if (!imageFileFolder.mkdir()) {
                    Log.i(TAG, "create file directory failed");
                    return false;
                }
            }
            return true;
        ...
        private File createImageFile() {
            ...
            if (!imageFile.exists()) {
                try {
                    Log.i(TAG, "created file:" + imageFile);
                    imageFile.createNewFile();
                } catch (IOException e) {
                    Log.i(TAG, "created file failed:" + imageFile);
                    e.printStackTrace();
                    return null;
                }
            }
        ...
        private File createMetaFile() {
            File metaFile = new File(mActivity.getExternalFilesDir(null), mFolder+"/"+mName+".txt");
            if (!metaFile.exists()) {
                try {
                    Log.i(TAG, "created file:" + metaFile);
                    metaFile.createNewFile();
                } catch (IOException e) {
                    Log.i(TAG, "created file failed:" + metaFile);
                    e.printStackTrace();
                    return null;
                }
            }
